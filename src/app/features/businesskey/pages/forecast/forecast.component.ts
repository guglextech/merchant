...
export class ForecastComponent extends PaginatedTableComponent {
  ...
  constructor(
    ...
    private businesskeySvc: BusinesskeyService,
  ...
  createForecastMessage(data: IForecastPayload) {
    this.businesskeySvc.createForecast(data).subscribe({
      next: (res) => {
  ...
  sendBroadcastSMS(data: any) {
    this.businesskeySvc.sendSmsMessage(SERVICE_TYPES.Finance, data.message ,data.status)
      .subscribe({
        next: (response: any) => {
